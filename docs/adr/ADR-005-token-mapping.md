...
const claimsToSubject = (claims: TokenClaims): Effect.Effect<Subject, ClaimsValidationError> =>
  Effect.gen(function* () {
...
const getCapabilities = (subject: Subject, claims: TokenClaims): ReadonlySet<Capability> => {
  switch (subject.kind) {
    case 'human':
    ...
    case 'agent':
      return getAgentCapabilities(subject, claims)
    case 'external':
      return externalCapabilityMap[subject.externalType] ?? new Set()
  }
}
