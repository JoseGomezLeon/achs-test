...
Given(
  'el operador de pagos ha solicitado el cierre del período {string}',
  async function (this: RbacWorld, period: string) {
...
Given(
  'no existe aprobación vigente para el período {string}',
  function (this: RbacWorld, _period: string) {
...
When(
  'el job {string} intenta ejecutar el cierre del período {string}',
  async function (this: RbacWorld, jobToken: string, period: string) {
...
Then('el cierre es ejecutado exitosamente', function (this: RbacWorld) {
  assert.equal(
    ...
    'granted',
    `se esperaba cierre exitoso, código: ${this.lastDenialCode}`
  )
})

Then(
  'queda registro de auditoría con actorKind {string}',
  function (this: RbacWorld, actorKind: string) {
    const entries = this.getAuditEntries()
    const found = entries.some((e) => e.actorKind === actorKind)
    assert.ok(
      found,
      `no se encontró entrada con actorKind="${actorKind}". Encontrados: ${[...new Set(entries.map((e) => e.actorKind))]}`
    )
  }
)
